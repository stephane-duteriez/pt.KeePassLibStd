...
#if NETSTANDARD2_0
        public static System.Drawing.Color ConvertHtmlToColor(String strColor)
        {
            if (strColor.StartsWith("#"))
                return Color.FromArgb(0xFF, Color.FromArgb(
			...
			switch(ctx)
			{
...
#if NETSTANDARD2_0
                            m_pwDatabase.Color = ConvertHtmlToColor(strColor);
#else
				...
				case KdbContext.Entry:
					...
					else if(xr.Name == ElemFgColor)
					{
						...
						if(!string.IsNullOrEmpty(strColor))
#if NETSTANDARD2_0
                            m_ctxEntry.ForegroundColor = ConvertHtmlToColor(strColor);
#else
					...
					else if(xr.Name == ElemBgColor)
					{
						...
						if(!string.IsNullOrEmpty(strColor))
#if NETSTANDARD2_0
                            m_ctxEntry.BackgroundColor = ConvertHtmlToColor(strColor);
#else
